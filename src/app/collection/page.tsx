...
import { ScanLine, PlusCircle, Trash2, Pencil, GripVertical, BookUp, Loader2, ChevronDown, Info } from 'lucide-react'
...
import { getBooks, addBook, deleteBook, saveCopy, deleteCopy, updateBookOrder, updateCopiesOrder, updateBookDetails } from '@/services/book-service'
...
export default function CollectionPage() {
  ...
  const [editingCopy, setEditingCopy] = useState<{ book: Book, copy: Copy } | null>(null)
  ...
  const [initialBookData, setInitialBookData] = useState<BookLookupOutput | null>(null)
  ...
  const fetchBooks = useCallback(async () => {
    try {
      ...
      toast({ title: 'Error Fetching Collection', description: (error as Error).message, variant: 'destructive' })
    } finally {
      setIsLoading(false)
    }
  }, [toast])
  ...
  const displayedBooks = useMemo(() => {
    ...
    if (searchTerm) {
        items = items.filter(book => book.title.toLowerCase().includes(searchTerm.toLowerCase()));
    }
  ...
  const handleLookup = async (isbnOverride?: string) => {
    ...
    if (!isbnToUse) {
      toast({ title: 'ISBN required', description: 'Please enter an ISBN to look up.', variant: 'destructive' })
      return
    }
  ...
  const handleAddBook = async (book: Omit<Book, 'id' | 'copies' | 'sortIndex'>) => {
    try {
        await addBook(book);
        await fetchBooks();
    } catch (error) {
        console.error('Failed to add book:', error);
        toast({ title: 'Error Adding Book', description: (error as Error).message, variant: 'destructive' });
        throw error; // Re-throw to be caught in the modal
    }
  }

  const handleUpdateBook = async (bookData: Omit<Book, 'id' | 'copies' | 'sortIndex'>) => {
    ...
    try {
      ...
      toast({ title: 'Error Updating Book', description: (error as Error).message, variant: 'destructive' });
      throw error; // Re-throw to be caught in the modal
    }
  };
  ...
  const handleSaveCopy = async (bookId: string, copy: Copy) => {
    try {
      ...
      toast({ title: 'Error Saving Copy', description: (error as Error).message, variant: 'destructive' })
    }
  };
  ...
  const handleDeleteBook = async (bookId: string) => {
    try {
      ...
      toast({ title: "Book Deleted", description: "The book and all its copies have been removed." });
    } catch (error) {
      console.error('Failed to delete book:', error)
      toast({ title: 'Error Deleting Book', description: (error as Error).message, variant: 'destructive' })
    }
  };
  ...
  const handleDeleteCopy = async (bookId: string, copyId: string) => {
    try {
        ...
        await fetchBooks()
        toast({ title: "Copy Deleted", description: "The copy has been removed." });
    } catch (error) {
        console.error('Failed to delete copy:', error)
        toast({ title: 'Error Deleting Copy', description: (error as Error).message, variant: 'destructive' })
    }
  }
  
  const handleToggleListed = async (bookId: string, copyId: string, isListed: boolean) => {
    ...
    try {
        ...
        const updatedBooks = books.map(b => b.id === bookId ? { ...b, copies: b.copies.map(c => c.id === copyId ? updatedCopy : c) } : b);
        setBooks(updatedBooks);
    } catch (error) {
        console.error('Failed to toggle listed status:', error)
        toast({ title: 'Error Updating Copy', description: (error as Error).message, variant: 'destructive' })
    }
  }
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (active.data.current?.type === 'book' && over.data.current?.type === 'book' && active.id !== over.id) {
      ...
      try {
          ...
          toast({ title: 'Error Saving Order', description: (error as Error).message, variant: 'destructive' });
    ...
    if (activeIsCopy && overIsCopy && active.id !== over.id) {
      ...
      const reorderedCopies = arrayMove(book.copies, oldCopyIndex, newCopyIndex);
      
      // Optimistic update
      setBooks(prev => prev.map(b => b.id === bookId ? { ...b, copies: reorderedCopies } : b));
  ...
  function SortableBookItem({ book }: { book: Book }) {
    ...
    return (
      <div ref={setNodeRef} style={style} {...attributes}>
        <AccordionItem value={book.id} className="border-border bg-card rounded-lg border overflow-hidden">
            <div className="flex w-full items-start p-4 text-left">
              <div {...(isManualSort ? listeners : {})} className={cn("touch-none flex-shrink-0 pt-1", isManualSort ? "cursor-grab" : "cursor-auto")}>
                <GripVertical className={cn("h-5 w-5 text-muted-foreground", !isManualSort && "opacity-30")} />
              </div>
              ...
              <div className="ml-4 flex-grow min-w-0">
                ...
                <div className="mt-2 flex flex-col gap-y-1 text-xs text-muted-foreground">
                  ...
                  <div className="flex flex-wrap items-center gap-x-2">
                    ...
                    {book.publishedYear && <span className="text-muted-foreground">&bull;</span>}
              ...
              <div className="ml-auto flex items-start gap-1 pl-2">
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <div className="flex items-center">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); openEditBookModal(book); }}>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This will permanently delete "{book.title}" and all of its copies from your collection. This action cannot be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>Cancel</AlertDialogCancel>
                              <AlertDialogAction onClick={() => handleDeleteBook(book.id)} className="bg-destructive hover:bg-destructive/80">Delete Book</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                    </div>
          ...
          <AccordionContent className="p-4 pt-0">
            <div className="border-t border-border pt-4 mt-4">
              <div className="flex justify-between items-center mb-4">
                <h4 className="font-headline text-lg font-semibold">Copies ({book.copies.length})</h4>
                <Button variant="outline" size="sm" onClick={() => openAddCopy(book)}>
              ...
              <div className="space-y-4">
                {book.copies.length > 0 ? (
                  <SortableContext items={book.copies.map(c => c.id)} strategy={verticalListSortingStrategy}>
                    {book.copies.map(copy => (
                      <SortableCopyItem key={copy.id} book={book} copy={copy} />
                    ))}
                  </SortableContext>
                ) : (
                  <p className="text-center text-muted-foreground py-4">No copies found. Add one!</p>
                )}
              </div>
            </div>
          </AccordionContent>
  ...
  function SortableCopyItem({ book, copy }: { book: Book, copy: Copy }) {
    const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: copy.id, data: { type: 'copy', bookId: book.id } });
    const style = { transform: CSS.Transform.toString(transform), transition };
    ...
    return (
      <div ref={setNodeRef} style={style} {...attributes} className="flex items-start gap-4 p-4 rounded-md border border-border/50 bg-background touch-none">
        ...
        <div className="flex-grow">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <p className="font-semibold text-muted-foreground">Price (Purchase/Market)</p>
              <p>${copy.purchasePrice?.toFixed(2) ?? 'N/A'} / ${copy.marketPrice?.toFixed(2) ?? 'N/A'}</p>
            </div>
            <div>
              <p className="font-semibold text-muted-foreground">Acquired</p>
              <p>{copy.purchaseDate ? copy.purchaseDate.toLocaleDateString() : 'N/A'} at {copy.purchaseLocation || 'N/A'}</p>
            </div>
          </div>
          {copy.notes && <div className="mt-4">
            <p className="font-semibold text-muted-foreground">Notes</p>
            <p className="text-sm text-foreground/80 whitespace-pre-wrap">{copy.notes}</p>
          </div>}

          {(financialMetrics.pnl !== null) && (
              <div className="mt-4 pt-4 border-t border-border/50">
                <h4 className="font-semibold text-sm text-muted-foreground mb-2">Profitability Analysis</h4>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className="text-xs text-muted-foreground">P/L</p>
                    <p className={`font-bold text-lg ${financialMetrics.pnl < 0 ? 'text-destructive' : 'text-green-500'}`}>
        ...
        <div className="flex flex-col items-end gap-4">
          <div className="flex items-center space-x-2">
            <Label htmlFor={`listed-${copy.id}`} className="text-sm text-muted-foreground">Listed</Label>
            <Switch
              id={`listed-${copy.id}`}
              checked={copy.isListed}
              onCheckedChange={(checked) => handleToggleListed(book.id, copy.id, checked)}
              className="data-[state=checked]:bg-primary"
            />
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" onClick={() => openEditCopy(book, copy)}>
              <Pencil className="h-4 w-4 text-accent" />
            </Button>
            <AlertDialog>
              <AlertDialogTrigger asChild>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This action cannot be undone. This will permanently delete this copy from your collection.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                  <AlertDialogAction onClick={() => handleDeleteCopy(book.id, copy.id)} className="bg-destructive hover:bg-destructive/80">Delete</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
          </div>
  ...
  const renderContent = () => {
    ...
    if (books.length === 0 && !isLoading) {
       return (
         <div className="text-center py-20">
            <h3 className="font-headline text-2xl font-bold">Your Collection is Empty</h3>
            <p className="text-muted-foreground mt-2">Start by adding your first book using the lookup or manual entry.</p>
         </div>
       )
    }
    ...
    return (
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <Accordion type="single" collapsible className="w-full space-y-4" value={openAccordionId} onValueChange={setOpenAccordionId}>
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="font-headline text-2xl font-bold tracking-wider text-primary">SCRYVAULT</h1>
            </div>
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="hidden sm:flex items-center gap-2">
                ...
                />
                <Button variant="outline" className="border-accent text-accent hover:bg-accent/10 hover:text-accent" onClick={() => handleLookup()} disabled={isLookingUp}>
                  {isLookingUp ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <BookUp className="mr-2 h-4 w-4" />}
                  Lookup
                </Button>
              </div>
              <Button variant="ghost" size="icon" className="text-accent sm:hidden" onClick={() => setScannerOpen(true)}>
                <ScanLine />
              </Button>
              <Button className="bg-primary hover:bg-primary/80" onClick={() => setManualAddOpen(true)}>
                <PlusCircle className="mr-2 h-4 w-4" />
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
          <h2 className="font-headline text-3xl font-bold shrink-0">Your Collection</h2>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full md:w-auto md:justify-end">
            ...
            <div className="flex items-center space-x-2 p-2 rounded-md border border-input sm:border-none sm:p-0">
              <Switch id="show-listed" checked={showListedOnly} onCheckedChange={setShowListedOnly} />
              <Label htmlFor="show-listed" className="whitespace-nowrap">Listed only</Label>
            </div>
          </div>
        </div>
        {renderContent()}
